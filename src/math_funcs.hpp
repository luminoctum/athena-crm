...
template<typename T>
inline T _rad2deg(T phi) { return phi*180./M_PI; }

template<typename T>
inline T _deg2rad(T phi) { return phi*M_PI/180.; }
...
template<typename T>
T _interp1(T x, T const *data, T const *axis, int len)
{
  T value;
  _interpn(&value, &x, data, axis, &len, 1);
  return value;
}
